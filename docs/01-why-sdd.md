---
summary: "The six problems with standard AI coding and how SDD solves each one. Plus: why a Go CLI handles the deterministic work so Claude doesn't have to."
read_when:
  - "Understanding SDD philosophy"
  - "Evaluating SDD for a project"
---
...
## Why a Go CLI? The Token Waste Problem

Before describing what SDD solves, it is worth explaining how it is built — because the
architecture directly determines the quality of the solution.

### The naive approach and its cost

A pure-LLM orchestration approach asks Claude to manage everything: read the filesystem,
track phase state, assemble context for sub-agents, run verification, and move artifacts
to the archive. This works, but it is expensive. Every byte the orchestrator reads and
every decision it makes about file paths and phase transitions burns context tokens.
Measured across a full SDD pipeline run, pure-LLM orchestration costs roughly 458K tokens.

### The `sdd` binary

SDD ships a Go CLI binary called `sdd`. It handles every deterministic operation that
does not require reasoning:

| Operation | Who does it | Token cost |
|-----------|-------------|-----------|
| State detection (what phase is next?) | `sdd` binary | 0 |
| Context assembly (which artifacts go to which sub-agent?) | `sdd` binary | 0 |
| Artifact path resolution | `sdd` binary | 0 |
| Cache lookup (is this context already assembled?) | `sdd` binary | 0 |
| Verify (run typecheck, lint, tests, parse results) | `sdd` binary | 0 |
| Archive (move files, merge specs, write manifest) | `sdd` binary | 0 |
| Quality gate enforcement (block archive if FAIL) | `sdd` binary | 0 |
| Exploration, proposal, spec, design, tasks, review | Claude sub-agents | tokens |

The result is a ~65% token reduction vs. pure-LLM orchestration: 458K → 161K tokens
measured on a representative full-pipeline run. Claude focuses entirely on the work
that requires reasoning. The `sdd` binary handles everything else at zero token cost.

### 14 patterns from the Go CLI ecosystem

The `sdd` binary architecture draws on 14 patterns from production Go CLIs.
Key patterns include: content-hash + TTL
caching for context assembly, structured JSONL output for pipeline state, atomic file
operations for artifact writes, and sub-process isolation with clean environment
inheritance for sub-agent launches.

### Mechanical enforcement beats instructions

An instruction to Claude ("do not skip the verify phase") can be forgotten, reasoned
around, or simply not applied when context is compressed. A Go gate that checks the
verify-report verdict before writing archive artifacts cannot be bypassed. SDD uses the
`sdd` binary for all quality gates precisely because mechanical enforcement is more
reliable than instruction-following at scale.
...
Each SDD sub-agent gets **only** the context relevant to its phase. The `sdd` binary
assembles this context before launching the sub-agent — reading artifact paths from the
pipeline state file, hashing the content for cache validation, and passing only the
relevant slice via environment:
...
The `sdd` binary caches assembled context by content-hash + TTL. If the same artifacts
are requested twice (e.g., when an apply phase retries after a build failure), the
assembled context is served from cache without re-reading files. Sub-agents receive
pre-assembled, cache-validated context — no token cost for the assembly itself.
...
Decisions are not stored in the conversation. They are written to files on disk by the
`sdd` binary and by Claude sub-agents as structured artifacts:
...
The `sdd` binary is the custodian of this artifact state. It knows which artifacts exist,
which are stale, and which gates are satisfied — without Claude needing to re-read
everything to determine pipeline state.
...
The `sdd` binary also maintains a Context Cascade: a cumulative decision log that is
automatically prepended to each phase's assembled context. Every architectural decision
made in prior phases is carried forward into subsequent phases without Claude needing to
re-summarize prior conversation history.
...
verify-report   →  Does the implementation pass quality gates?
                    (typecheck, lint, tests, security — run by sdd binary)
        |
        v
archive/        →  Historical record with learnings
                    (merged specs, captured patterns, noted risks — archived by sdd binary)
```

The verify and archive steps are 100% Go operations executed by the `sdd` binary. No
Claude tokens are consumed verifying that the build passes or moving artifacts to the
archive directory. Quality gates are enforced mechanically: the binary reads the
verify-report verdict and blocks archive if the verdict is FAIL.
...
What SDD changes is the **architecture** of the interaction — and crucially, which parts
of that interaction are handled by Claude versus by the `sdd` Go binary:
...
| LLM manages all state and verification | `sdd` binary handles state, verify, archive at zero token cost |
| 458K tokens for full pipeline | ~161K tokens (65% reduction) |

The same model produces dramatically better results when given constraints, traceability,
and fresh context at each step. The `sdd` binary provides the harness that makes those
conditions reliable and efficient. SDD is the workflow that provides those conditions.
